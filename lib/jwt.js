...
async function refreshToken(req, res){
    if(req.headers.authorization){
        const [refreshTokenType, refreshTokenValue] = req.headers.authorization.split(' ');
        ...
        if(refreshTokenType !== 'Bearer')
            ...
            return res.status(400).json({ result: false, message: '요청타입이 올바르지 않습니다.'});

        const verifyRefreshToken = await refreshTokenVerify(refreshTokenValue, decodeRefreshToken.userId);
        if(!verifyRefreshToken)
            //refreshToken을 검사한 뒤, 해당 refreshToken이 만료 또는 정상이 아니라면 다시 로그인시킨다.
            return res.status(401).json({ result: false, message: 'refreshToken이 만료되었거나, 유효하지 않습니다. 재로그인이 필요합니다.' });
        ...
        if(!updateResult)
            return res.status(400).json({ result: false, message: 'refreshToken Update를 수행하지 못했습니다.'});

        return res.status(200).json({ result: true, message: 'accessToken 재발행 정상', data: { accessToken: newAccessToken, refreshToken: newRefreshToken } });
        
    }else{
        // accessToken과 refreshToken 2개 중 하나라도 들어오지 않은 경우 실패처리.
        return res.status(400).json({ result: false, message: '헤더 내 refreshToken 값이 존재하지 않습니다.'});
    }
}
