    ...
    try {
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            for (let j = 0; j < people.length; j++) {
                ...
                let joinedUser = await User.find({ userId: people[j].studyMemberId })
...
async function updateStudy(req, res) {
    ...
    try {
        ...
        return res.status(201).json({ result: 'true', message: '스터디 정보 수정 완료!' })
    } catch (err) {
        console.log(err)
        res.status(400).json({ result: 'false', message: '스터디를 수정할 수 없습니다!' })
    };
...
async function joinStudy(req, res) {
    ...
    if (studyType === 'join') {
        ...
        try {
            let study = await STUDY.findOne({ studyId })
            if (study.masterId === userId) {
                return res.status(400).json({ result: 'false', message: '스터디장은 나갈 수 없습니다.' })
            }
