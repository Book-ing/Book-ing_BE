...
async function getKakaoLoginCallback(req, res){
    ...
    try {
        ...
        console.log(error);
        return res.status(400).json({result: false, message: '카카오 accessToken 취득실패 (사유: '+error.message+')'});
    }
    ...
    try {
        ...
        getKakaoUserInfo = await axios({
            ...
            headers:{ Authorization: `Bearer ${getKakaoAccessToken.data.access_token}` }
        });
    } catch (error) {
        // 어떠한 사유로 인해 사용자정보를 받지 못했다면, 서버에 로그를 남기고, 해당 메시지를 클라이언트에 내려준다.
        console.log(error);
        console.log(getKakaoAccessToken.data.access_token);
        return res.status(400).json({result: false, message: '카카오 사용자정보 취득실패 (사유: '+error.message+')'});
    }

    // 카카오 서버에서 받아온 카카오 사용자 정보 내 kakaoUserId로 DB 내 가입이력이 있는지 검사한다.
    const existUser = await USER.findOne({ kakaoUserId: getKakaoUserInfo.data.id });
    if(existUser){
        // 가입이력이 있는 경우

        //JWT Token을 생성하여 내려준다.
        const payload = { kakaoUserId: existUser.kakaoUserId, username: existUser.username, userId: existUser.userId };
        bookingAccessToken = jwt.sign(payload, process.env.ACCESS_TOKEN, { expiresIn: process.env.VALID_ACCESS_TOKEN_TIME });
        bookingRefreshToken = jwt.sign(payload, process.env.REFRESH_TOKEN, { expiresIn: process.env.VALID_REFRESH_TOKEN_TIME });

        //카카오서버에서 받아온 사용자정보(닉네임, 프로필사진)를 업데이트시켜준다.
        let username = getKakaoUserInfo.data.kakao_account.profile.nickname;
        let profileImage = getKakaoUserInfo.data.kakao_account.profile.profile_image_url;
        await USER.updateOne({ kakaoUserId: getKakaoUserInfo.data.id }, { $set: { username, profileImage, bookingRefreshToken } });
        ...
        const newUser = await USER.create({
            ...
            profileImage: getKakaoUserInfo.data.kakao_account.profile.profile_image_url,
            refreshToken: 'temp', // FIXME: 사용자가 생성될 당시에는 refresh token이 존재하지 않기 때문에 null 값을 허용해야한다. (호진님께 요청함)
            regDate: lib.getDate()
        });

        //JWT Token을 생성하여 내려준다.
        const payload = { kakaoUserId: newUser.kakaoUserId, username: newUser.username, userId: newUser.userId };
        bookingAccessToken = jwt.sign(payload, process.env.ACCESS_TOKEN, { expiresIn: process.env.VALID_ACCESS_TOKEN_TIME });
        bookingRefreshToken = jwt.sign(payload, process.env.REFRESH_TOKEN, { expiresIn: process.env.VALID_REFRESH_TOKEN_TIME });
    }

    res.cookie('accessToken', bookingAccessToken, { sameSite: 'None', secure: true, httpOnly: true });
    res.cookie('refreshToken', bookingRefreshToken, { sameSite: 'None', secure: true, httpOnly: true });
